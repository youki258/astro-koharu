...
频道中的所有消息默认全部展示。若只想发布以某个 hashtag 开头的消息（例如 #碎碎念），配置：
...
启用后，碎碎念页面、站内搜索、全局与频道 RSS 只会展示正文以配置 hashtag 开头的消息，未匹配消息的详情
直链返回 404，上下文导航也会跳过它们。套帥（album）内任意成员匹配则整组保留，避免拆散相册。suite 侧仍然
完整归档，随时可修改或删除该配置，不影响任何已采集数据。

匹配规则：对正文 `content.text` 做去空白后的开头匹配；hashtag 之后的字符必须是标签边界（非字母、数字或
下划线），因此 `#碎碎念2` 不会命中 `#碎碎念`；纯媒体消息（无正文）不匹配。suite 按“原始消息”分页，启用
过滤后渲染会自动多拉几页 cursor 凑满当前页（封顶 8 页），因此每页条数接近正常，但多页拉取会增加少量延迟；
若达到上限且没有匹配消息，页面可能显示为空，继续翻页即可。此过滤只影响 astro-koharu 的展示层，suite
不提供也不需要该能力。
