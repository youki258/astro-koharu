...
既定ではチャンネルの全メッセージを表示します。特定の hashtag で始まるメッセージだけを公開したい場合（例：#碎碎念）は次のように設定します：
...
有効にすると、ひとことのページ、サイト内検索、全体／チャンネル RSS は、本文が指定 hashtag で始まるメッセージのみを
表示します。一致しないメッセージの詳細リンクは 404 を返し、前後ナビゲーションもスキップします。アルバムのどれかの
メンバーが一致すればグループ全体を保持し、アルバムを分割しません。suite 側は完全なアーカイブを続けるため、この設定の
変更・削除は収集済みデータに影響しません。

一致ルール：`content.text` の前後空白を除いた先頭が hashtag と一致し、その直後の文字がタグ境界（英数字・アンダースコア
以外）であること。`#碎碎念2` は `#碎碎念` に一致しません。本文のないメディアのみのメッセージは一致しません。suite は生
メッセージ単位でページングするため、描画時に cursor を最大 8 ページ追加取得して表示ページを埋めます。上限に達して一致が
見つからない場合、ページが空に見えることがあり、そのまま次へ進めます。このフィルターは astro-koharu の表示層のみに影響し、
suite には必要ありません。
