...
上述错误信息的含义是，`emp`员工表的`phone`手机号字段的值重复了，因为在数据库表`emp`中已经有了`138xxxx0027`这个手机号了，我们之前设计这张表时，为`phone`字段建议了唯一约束，所以该字段的值是不能重复的。

而当我们再将该员工的手机号也设置为 `138xxxx0027`，就违反了唯一约束，此时就会报错。
...
重新启动 SpringBoot 服务，打开浏览器，再来测试一下 修改员工 这个操作，我们依然设置已存在的 `138xxxx0027`这个手机号：
