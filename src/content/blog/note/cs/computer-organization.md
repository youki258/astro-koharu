...
| 编码方式 | 正数 | 负数 | 特点 |
|---------|------|------|------|
| 原码 | 符号位 0 + 真值绝对值 | 符号位 1 + 真值绝对值 | 有+0 和-0 |
| 反码 | 与原码相同 | 符号位不变，数值位取反 | 有+0 和-0 |
| 补码 | 与原码相同 | 反码+1 | 只有一个 0，范围更大 |

**补码的优势**：

1. **消除正负零**：原码和反码中+0 和-0 表示不同，补码中只有一个 0
2. **加减法统一**：减法可以转化为加法运算
3. **表示范围扩大**：8 位补码可表示 -128 ~ +127
...
- MAR = 4 位 → 共有 2^4 = 16 个存储单元
- MDR = 16 位 → 每个存储单元可存放 16bit
- 1 个字（word）= 16bit
- 1 字节（Byte）= 8bit，1B = 1Byte，1b = 1bit
