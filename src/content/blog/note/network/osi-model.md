...
![OSI七层](./remote-02.webp)

OSI 定义了网络互连的七层框架，即 ISO 开放互连系统参考模型。
...
![TCP/IP模型](./remote-03.webp)

- **应用层**：处理用户与网络应用程序之间的通信（HTTP、FTP、SMTP 等）
- **传输层**：提供端到端的数据传输服务（TCP、UDP）
- **网络层**：负责数据包的路由和转发（IP 协议）
...
![计算机网络的组成](./part2-remote-04.webp)

![计算机网络的功能](./part2-remote-05.webp)

## 6. 电路交换、报文交换、分组交换

![电路交换](./part2-remote-11.webp)

![分组交换与报文交换](./part2-remote-12.webp)

### 6.1 电路交换 vs 分组交换时延计算

**条件**：报文共 x(bit)，源到终点经 k 段链路，每段传播时延 d(s)，带宽 b(bit/s)。电路交换建立时间 s(s)。分组交换将报文划分为长度 p(bit) 的分组（首部长度和排队时间忽略不计）。

![电路交换时延](./switching-delay-01.webp)

![分组交换时延](./switching-delay-02.webp)
...
![最优分组长度推导](./switching-delay-03.webp)

令 dD/dp = 0，解得 **p = √(xh/(k−1))**

## 7. 计算机网络分类

按覆盖范围分类：**局域网（LAN）**覆盖较小区域（如办公室、校园）；**城域网（MAN）**覆盖城市范围；**广域网（WAN）**覆盖国家或全球范围；**个人区域网（PAN）**覆盖个人周围约 10 米范围。按拓扑结构可分为总线型、星型、环型、网状型等。

![计算机网络分类](./part2-remote-21.webp)
...
![性能指标-带宽与速率](./part2-remote-28.webp)

![性能指标-时延](./part2-remote-29.webp)
